    ...
    <div class="card-body">
        {!! Form::open(array('route' => ['users.update', $user->id], 'method' => 'PUT', 'role' => 'form', 'class' => 'needs-validation')) !!}
                        ...
                        <select multiple="multiple" class="form-control" name="role[]" id="role">
                            ...
                            @if ($roles)
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}"  @if(in_array($role->id, $currentRole)) selected="selected" @endif >{{ $role->name }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
            ...
            <div class="form-group has-feedback row {{ $errors->has('majlis_id') ? ' has-error ' : '' }}">
                <?php $majlis_ids = json_decode($user->majlis_id); ?>
                    ...
                    <div class="input-group">
                        <select multiple="multiple" class="form-control" name="majlis_id[]">
                            ...
                            @foreach($majlis as $mjl)
                              <option value="{{$mjl->id}}" @foreach($majlis_ids as $maj) @if($mjl->id == $maj) selected="selected" @endif @endforeach>{{$mjl->name}}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="form-group has-feedback row {{ $errors->has('tanzeem_id') ? ' has-error ' : '' }}">
                <?php $tanzeem_ids = json_decode($user->tanzeem_id); ?>
                    ...
                    <div class="input-group">
                        <select multiple="multiple" class="form-control" name="tanzeem_id[]">
                            ...
                            @foreach($tanzeems as $tanzeem)
                              <option value="{{$tanzeem->id}}" @foreach($tanzeem_ids as $tan) @if($tanzeem->id == $tan) selected="selected" @endif @endforeach>{{$tanzeem->name}}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="form-group has-feedback row {{ $errors->has('verify_code') ? ' has-error ' : '' }}">
                ...
                <div class="col-md-9">
                    <div class="input-group">
                        {!! Form::text('verify_code', $user->verify_code, array('id' => 'verify_code', 'class' => 'form-control', 'placeholder' => 'AMI Member Code')) !!}
                    </div>
